...
class OftalmologosController(DefaultValues):
    def __init__(self):
        DefaultValues.__init__(self)
        self.modelo_name = 'Oftalmologos'
        self.modelo_id = 'oftalmologo_id'
        self.modelo_app = 'configuraciones'
        self.modulo_id = settings.MOD_OFTALMOLOGOS

        # variables de session
        self.modulo_session = "oftalmologos"
        self.columnas.append('oftalmologo')
        self.columnas.append('codigo')

        self.variables_filtros.append('search_oftalmologo')
        self.variables_filtros.append('search_codigo')

        self.variables_filtros_defecto['search_oftalmologo'] = ''
        ...
        self.control_form = "txt|2|S|oftalmologo|Oftalmologo"
    ...
    def index(self, request):
        ...
        # oftalmologo
        if self.variables_filtros_values['search_oftalmologo'].strip() != "":
            self.filtros_modulo['oftalmologo__icontains'] = self.variables_filtros_values['search_oftalmologo'].strip()
    ...
    def is_in_db(self, id, nuevo_valor):
        ...
        filtros['oftalmologo__in'] = [nuevo_valor]
        if id > 0:
    ...
    def save(self, request, type='add'):
        """aniadimos una nueva oftalmologo"""
        system_controller = SystemController()

        try:
            oftalmologo_txt = validate_string('oftalmologo', request.POST['oftalmologo'], remove_specials='yes')
            ...
            if not self.is_in_db(id, oftalmologo_txt):
                if 'activo' in request.POST.keys():
                    status_oftalmologo = self.status_activo
                else:
                    status_oftalmologo = self.status_inactivo
                ...
                if 'imagen1' in request.FILES.keys() and request.FILES['imagen1'].name.strip() != '':
                    ...
                    aux = system_controller.nombre_imagen('oftalmologos', uploaded_filename)

                    full_filename = os.path.join(settings.STATIC_ROOT_APP, 'media', 'oftalmologos', aux['nombre_archivo'])
                    full_filename_thumb = os.path.join(settings.STATIC_ROOT_APP, 'media', 'oftalmologos', aux['nombre_archivo_thumb'])
                    ...
                    if type == 'add':
                        aux = system_controller.nombre_imagen('oftalmologos', settings.PRODUCTOS_NO_IMAGE)
                        full_filename = os.path.join(settings.STATIC_ROOT_APP, 'media', 'oftalmologos', aux['nombre_archivo'])
                        full_filename_thumb = os.path.join(settings.STATIC_ROOT_APP, 'media', 'oftalmologos', aux['nombre_archivo_thumb'])
                ...
                if type == 'add':
                    oftalmologo = apps.get_model('configuraciones', 'Oftalmologos').objects.create(oftalmologo=oftalmologo_txt, codigo=codigo_txt,
                                                                                                   descripcion=descripcion, imagen=aux['nombre_archivo'], imagen_thumb=aux['nombre_archivo_thumb'], status_id=status_oftalmologo, created_at='now', updated_at='now')
                    oftalmologo.save()
                    self.error_operation = ""
                    return True

                if type == 'modify':
                    oftalmologo = apps.get_model('configuraciones', 'Oftalmologos').objects.get(pk=id)
                    # verificamos imagen
                    if aux['nombre_archivo'] != '':
                        # se cambio de imagen
                        if oftalmologo.imagen != '':
                            full_filename = os.path.join(settings.STATIC_ROOT_APP, 'media', 'oftalmologos', oftalmologo.imagen)
                            full_filename_thumb = os.path.join(settings.STATIC_ROOT_APP, 'media', 'oftalmologos', oftalmologo.imagen_thumb)
                        ...
                        oftalmologo.imagen = aux['nombre_archivo']
                        oftalmologo.imagen_thumb = aux['nombre_archivo_thumb']

                    # datos
                    oftalmologo.status_id = status_oftalmologo
                    oftalmologo.oftalmologo = oftalmologo_txt
                    oftalmologo.codigo = codigo_txt
                    oftalmologo.descripcion = descripcion
                    oftalmologo.updated_at = 'now'
                    oftalmologo.save()
                    self.error_operation = ""
                    return True
                ...
                self.error_operation = "Ya existe esta oftalmologo: " + oftalmologo_txt
                return False

        except Exception as ex:
            self.error_operation = "Error al agregar la oftalmologo, " + str(ex)
            return False
