...
def rptVentas(buffer_pdf, usuario, ciudad_id, sucursal_id, punto_id, fecha_ini, fecha_fin, anulados, preventa, venta, finalizado):
    ...
    datos_reporte = reporte_controller.datos_ventas(usuario, ciudad_id, sucursal_id, punto_id, fecha_ini, fecha_fin, anulados, preventa, venta, finalizado)
    # print(datos_reporte)
    ...
    for dato in datos_reporte:
        ...
        if punto_id != dato['punto']:
            ...
            if bande > 1:
                ...
                if len(data) > 0:
                    ...
                    # ancho columnas
                    tabla_datos = Table(data, colWidths=[22*mm, 12*mm, 58*mm, 25*mm, 17*mm, 15*mm, 17*mm, 17*mm], repeatRows=1)
                    tabla_datos.setStyle(TableStyle([('BACKGROUND', (0, 0), (7, 0), colors.Color(red=(204/255), green=(204/255), blue=(204/255))),
                                                     ('ALIGN', (4, 0), (7, filas+1), 'RIGHT'),
                                                     #('ALIGN', (1, 0), (1, filas+1), 'RIGHT'),
                                                     #('ALIGN', (5, filas+1), (5, filas+1), 'RIGHT'),
                                                     ('GRID', (0, 0), (-1, -1), 0.5, colors.black),
                                                     ('FONTSIZE', (0, 0), (7, 0), 10),
                                                     ('FONTSIZE', (0, 1), (-1, -1), 9),
                                                     ('FONTNAME', (0, 0), (7, 0), 'Helvetica'),
                                                     ('FONTNAME', (0, 1), (7, filas), 'Helvetica'),
                                                     ('FONTNAME', (0, filas+1), (7, filas+1), 'Helvetica-Bold'),
            ...
            data.append(['Fecha', 'N.', 'Cliente', 'Tipo', 'SubT', 'Desc', 'Total', 'Ingr.'])
            filas = 0
            total = 0
            subtotal = 0
            descuento = 0
        ...
        datos_tabla = [dato['fecha'], str(dato['numero_venta'])+' ', cliente, tipo, str(dato['subtotal']), str(dato['descuento']),
                       str(dato['total']), str(dato['ingresos_caja'])]
        data.append(datos_tabla)
        filas += 1
