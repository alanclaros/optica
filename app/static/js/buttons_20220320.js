...
function openModule(module_id) {
	...
	const module_ref = document.getElementById('module_ref_' + module_id);
...
$(document).ready(function () {
	...
	window.addEventListener('popstate', function (e) {
		...
		if (e.state) {
			...
			console.log('modulo cargar: ', moduloCargar);
...
function sendOperation(operation = '', operation2 = '', operation3 = '', id = '', id2 = '', id3 = '') {
	...
	div_modulo.load(hostURL, datos_operation, function () {
		//termina de cargar la ventana
		window.history.pushState({ href: module_x + '/' + operation }, '', module_x + '/' + operation);
	});
}
